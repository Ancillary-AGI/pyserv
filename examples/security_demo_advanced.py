#!/usr/bin/env python3
"""
Quantum Security Demo for Pyserv  Framework
...
from pyserv.security.cryptography import get_crypto_manager
from pyserv.security.key_management import get_key_manager, HSMConfig
...
async def demo_middleware_integration():
    ...
    try:
        from pyserv.core.security_middleware import UnifiedSecurityMiddleware

        print("\n1. Testing quantum security middleware availability...")
        print("   ✓ QuantumSecurityMiddleware imported successfully")

        # Create middleware instance with quantum features enabled
        config = UnifiedSecurityMiddleware.__init__.__defaults__[0] or UnifiedSecurityMiddleware.SecurityConfig()
        config.quantum_auth_enabled = False  # Optional for demo
        config.quantum_channel_enabled = True

        middleware = UnifiedSecurityMiddleware(config)
        print("   ✓ UnifiedSecurityMiddleware instance created with quantum features")

        # Show middleware configuration
        print("\n2. Middleware configuration:")
        print(f"   Quantum Auth Enabled: {config.quantum_auth_enabled}")
        print(f"   Quantum Channel Enabled: {config.quantum_channel_enabled}")

        # Get quantum manager status (if available)
        try:
            from pyserv.security.quantum_security import get_quantum_security_manager
            quantum_manager = get_quantum_security_manager()
            status = quantum_manager.get_security_status()
            print(f"   Active Security Agents: {status['active_agents']}")
        except ImportError:
            print("   Quantum security manager not available")
...
async def main():
    """Main demo function"""
    print("🚀 Pyserv  Quantum Security Framework Demo")
    print("=" * 60)
    ...
    try:
        ...
        print("   • Seamless integration with existing Pyserv  security modules")
